...
export type DeleteAMilestoneFileEffectArgs = {userId: number, offerId: number, milestoneId: number, hash: string, bucket: string}
export const deleteAMilestoneFileEffect = ({userId, offerId, milestoneId, hash, bucket} : DeleteAMilestoneFileEffectArgs) =>
  T.gen(function* () {
    ...
    return T.tryPromise({
              try: () => offerOperations.getAnOfferByCreatorId(offerId, userId),
                ...
                T.flatMap(() => T.tryPromise({
                    try: () =>
                    transactionOperations.deleteAMilestoneFile(milestoneId, hash),
                    catch: error => {
                    logger.error({
                        cause: 'database_error',
                ...
                T.flatMap(() => T.tryPromise({
                    try: () =>
                        storage
                        .getFileInfo(bucket)(rebuildPathForSecurity(userId, hash)),
                    catch: error => {
                        logger.error({
